...
public class FilesSpoutUnique extends BaseRichSpout {

	private static final long serialVersionUID = 1L;
	private SpoutOutputCollector collector;
	public String mainDirName;// main Dir name is like as ".../XXX/"
	transient Calendar cal = null;
	public SimpleDateFormat format = new SimpleDateFormat("/yyyyMMdd/HHmm/");
	public int lastT = -1;
	public SimpleDateFormat ftime = new SimpleDateFormat("mm");
	Boolean first = false;
	static Logger log = Logger.getLogger(FilesSpoutUnique.class);

	public FilesSpoutUnique(String mainDirName) {
	...
	public void nextTuple() {
		...
		if (min % TimeConst.PERIODUNIQUE == 0) {
			first = true;
		}
		...
		if ((timedif != 0) && (first)) {
			...
			if (path.exists()) {
				...
				try {
					...
					log.info("[ERROR] secondNeed pv read file fail.");
				}
			}
	...
	private void buildStartTime() {
		if (cal == null) {
			log.info("fileSpout1min初始化Calendar cal！");
			cal = STimePeriod.buildCal();
		}
	}
