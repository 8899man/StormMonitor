...
public class FilesReadPeriod extends BaseRichSpout  {
	...
	Boolean first = false;
	static Logger log = Logger.getLogger(FilesReadPeriod.class);

	public FilesReadPeriod(String mainDirName) {
	...
	public void nextTuple() {
		...
		lastT = min;
		if ((timedif != 0) && (first)) {
			log.info("1 min coming:" + min + "  lastT:" + lastT + " timedif: "
					+ timedif + "  first:" + first);
			...
			if (path.exists()) {
				...
				try {
					...
					log.info("[ERROR] read file fail.");
				}
			}
			log.info("file read end.\nPath：" + pathname);
		}
		try {
			Thread.sleep(TimeConst.FileTime);
	...
	private void buildStartTime() {
		if (cal == null) {
			log.info("fileSpoutPeriod initialize the Calendar cal！");
			cal = STime.buildCal();
		}
	}
