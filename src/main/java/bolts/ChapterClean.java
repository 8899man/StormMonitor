...
public class ChapterClean extends BaseRichBolt {

	private static final long serialVersionUID = 1L;
	OutputCollector collector;
	Set<String> visitSet = new HashSet<String>();
	static Logger log = Logger.getLogger(ChapterClean.class);
	...
	public void execute(Tuple input) {
		try {
			...
			String key = buildkey(msisdn, book_id, chapter_id);
			Boolean isvisited = isVisted(key);
			if (!isvisited) {
				visitSet.add(key);
				collector.emit(new Values(book_id));
			}
	...
	private String buildkey(String msisdn, String book_id, String chapter_id) {
		return msisdn + "|" + book_id + "|" + chapter_id;
	}
	...
	public void declareOutputFields(OutputFieldsDeclarer declarer) {
		declarer.declare(new Fields(FName.BOOK_ID.name()));
	}
