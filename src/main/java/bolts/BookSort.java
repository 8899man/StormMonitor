...
public class BookSort extends BaseRichBolt {
	...
	static Logger log = Logger.getLogger(BookSort.class);

	public BookSort(String startTime) {
	...
	public void execute(Tuple input) {
		String itemid = null;
		try {
			itemid = input.getStringByField(FName.ITEM_ID.name());
			...
			if (streamId.equals(StreamId.SIGNALSORT.name())) {
				log.info("signalsSort is coming!");
	...
	private void emitData() {
		...
		if (itemSort == null) {
			...
			log.info("TotalSZ is：" + sz);
			if (sz > TimeConst.TOPN) {
				sz = TimeConst.TOPN;
			}
			log.info("After comparing to TOPN. TotalSZ is ：" + sz);
			for (int i = 0; i < sz; i++) {
